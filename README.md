...
- `scripts/strava_sync.py` – Manual sync entrypoint for Strava or Komoot; `--source strava|komoot`, `--activity-ref "<exact Strava name or Komoot URL>"`, optional `--project "Name"` or `--project "Name #RRGGBB"`, and `--destination sommets|bikepacking` select source + sheet target; pushes GPX to `data/raw/` or `data/bike/raw/`. When set, project is written to bike column **G** or summits column **O** (when empty). A trailing hex (including `#`) is used as the `projectColors` track color; without a hex, new projects get placeholder `#E67E22`. Blank project leaves the sheet cell empty (map `defaultColor` / No Project). Bike destinations also join `bikeOnlyProjects`. For `source=komoot` on Sommets, all rows sharing the same tour URL in column `P` are updated; OSM altitude/coordinates use each row’s summit name in column `D`, and planned status (`à gravir` / `à faire`) in column `C` is cleared.
...
- `.github/workflows/strava_sync.yml` – Manual trigger: source (`strava` / `komoot`) + activity reference + optional **project** (`Name` or `Name #RRGGBB`) + destination (`sommets` / `bikepacking`); commits processed GeoJSON and any `script.js` color updates
...
- Project colors defined in `projectColors` in `script.js`; markers and bike tracks colored by Project, summit tracks by activity type. Manual sync can set color via `Name #RRGGBB`; name-only new projects get `#E67E22` until you change them. Blank project → No Project / `defaultColor`.
